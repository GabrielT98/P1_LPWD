...
public class TelefoneController {
    ...
    public ResponseEntity<Telefone> Put(@PathVariable(value = "id") long id, @Valid @RequestBody Telefone newTelefone) {
        Optional<Telefone> oldTel = _telefoneRepository.findById(id);
        if (oldTel.isPresent()) {
            Telefone telefone = oldTel.get();
            telefone.setCelular(newTelefone.getCelular());
            telefone.setFixo(newTelefone.getCelular());
